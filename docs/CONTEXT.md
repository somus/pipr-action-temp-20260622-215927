...
The Review Kernel computes the Diff Manifest once per Review Workflow run and shares it across Agent calls. Independent Agent steps may run in parallel when workflow `steps.*` dependencies allow it; the reserved `review` step remains the final validated review used for publication.

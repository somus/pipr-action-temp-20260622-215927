...
pipr uses repository-local `.pipr/` files as the executable product control plane. Runtime loads final materialized component files directly from conventional folders such as `workflows`, `agents`, `comments`, optional user `schemas`, and optional `blocks`.

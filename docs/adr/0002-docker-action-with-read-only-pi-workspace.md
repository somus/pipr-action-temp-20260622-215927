...
The Docker Action also treats PR-authored `.pipr/` changes as untrusted for runtime authority. Provider backend/model/API-key env come from trusted Action inputs, the review graph comes from pipr runtime code, and reviewer agent plus Main Review Comment template content is loaded from the pull request base commit. `config-dir` must resolve inside the repository root.

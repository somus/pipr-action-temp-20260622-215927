# TypeScript pipr config control plane
...
For pull request Action runs, pipr loads `.pipr/config.ts` and local imports from the base commit. Pi still reviews the pull request head. This keeps PR-authored config changes visible in the Diff Manifest but unable to change runtime authority for the current run.

Runtime-owned internals use `core/*` names for schemas and deterministic review/publication behavior, including `core/pr-review`. Users can compose tasks and agents in TypeScript, but diff creation, Pi adapter policy, model fallback and retry orchestration, review validation, stale-head checks, marker dedupe, and GitHub publication authority remain inside pipr. Custom plugin tools can be registered as typed handles, but Pi custom tool execution fails closed in the MVP.

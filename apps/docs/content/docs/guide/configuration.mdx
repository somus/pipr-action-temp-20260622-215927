...
Use `pipr.review(...)` for the default path: one reviewer, one Review Task, optional path scope, optional command, optional local entrypoint, capped inline comments, and optional task check.
...
```ts twoslash
import type { ReviewEntrypoints } from "@pipr/sdk";
...
Use [Entrypoints](/docs/guide/entrypoints) for command arguments, permissions, and local names.
...
| Local-only development task | `entrypoints.local` or `pipr.local(...)` |
